...
use crate::ui::{RawScreen, RawScreenInput, RawScreenCallback};
use crate::style::{StyledLine, err_line};
use crate::signal;
use crate::userinput;
...
pub struct Client {
    ...
    // to_server: Option<Sender<Packet>>,
}
...
impl Client {
    ...
    pub fn start_to_server_handle(&mut self, mut to_server: impl io::Write + Send + 'static) -> Result<Sender<Packet>> {
                ...
                match rx.recv() {
                    ...
                    Ok(sm) => {
                        if let Err(e) = Packet::from(sm).write_to(&mut to_server) {
                            ...
                            uitx.send(RawScreenInput::Line(err_line("无法向服务器发送信息，请尝试关闭应用并重新连接".to_owned()))).unwrap();
                            return;
                        }
                    }
                }
            ...
            loop {
                match Packet::read_from(&mut from_server) {
                    Err(e) => {
                        ...
                        uitx.send(RawScreenInput::Line(err_line("无法从服务器接收信息，请尝试关闭应用并重新连接".to_owned()))).unwrap();
                        return;
                    }
                    Ok(Packet::StyledText(spans, ended)) => {
                        ...
                        uitx.send(RawScreenInput::Line(StyledLine{spans, orig, ended})).unwrap();
                    }
